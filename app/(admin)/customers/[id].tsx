        ...
        <View style={styles.container}>
          <View style={styles.card}>
             ...
             <View style={{ flexDirection: 'row', gap: 10, marginTop: 20 }}>
                <TouchableOpacity style={[styles.btn, { flex: 1, backgroundColor: '#ccc' }]} onPress={() => setEditing(false)}>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList 
        ...
        renderItem={({ item }) => (
            <TouchableOpacity style={styles.jobRow} onPress={() => router.push(`/(admin)/jobs/${item.id}`)}>
                <View>
                    <Text style={styles.jobTitle}>{item.title}</Text>
                    <Text style={styles.jobDate}>{new Date(item.scheduled_date).toLocaleDateString()}</Text>
                </View>
        ...
        ListEmptyComponent={<Text style={{ color: '#888', fontStyle: 'italic' }}>No jobs for this customer yet.</Text>}
      />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', padding: 20, borderRadius: 12, marginBottom: 20, ...Colors.shadow },
  ...
  jobRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fff', padding: 16, borderRadius: 10, marginBottom: 10, borderWidth: 1, borderColor: '#eee' },
  jobTitle: { fontSize: 14, fontWeight: '600', color: Colors.text },
  jobDate: { fontSize: 12, color: '#888' },
  status: { fontSize: 11, fontWeight: '700' },
  label: { fontSize: 12, fontWeight: '700', color: Colors.textLight, marginBottom: 6, marginTop: 10 },
  input: { backgroundColor: '#f8fafc', padding: 10, borderRadius: 8, borderWidth: 1, borderColor: Colors.border },
