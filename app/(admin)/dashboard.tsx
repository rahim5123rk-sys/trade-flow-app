...
export default function DashboardScreen() {
  ...
  const stats = useMemo(() => {
    ...
    // Revenue: jobs that are marked as paid (either status or payment_status)
    const paidJobs = allJobs.filter(
      (j) => j.status === 'paid' || j.payment_status === 'paid'
    );
    const totalRevenue = paidJobs.reduce((sum, j) => sum + (j.price || 0), 0);
    ...
    // Upcoming: future date, not finished
    ...
    return {
      ...
      totalRevenue,
      upcomingJobs,
      outstandingAmount: outstandingJobs.reduce(
        (sum, j) => sum + (j.price || 0),
        0
      ),
    };
      ...
      <View style={styles.header}>
        <View>
          <Text style={styles.greeting}>
            Hello, {userProfile?.display_name?.split(' ')[0] || 'there'}
          </Text>
          <Text style={styles.subtext}>
            {role === 'worker'
              ? 'Viewing as Worker (Test Mode)'
...
const styles = StyleSheet.create({
  ...
  gridContainer: { gap: 12, marginBottom: 30 },
