...
function FgaGroup({label, value, onChange}: {label: string; value: {co: string; co2: string; ratio: string}; onChange: (value: {co: string; co2: string; ratio: string}) => void;}) {
  return (
    <View style={styles.fgaGroup}>
      <Text style={styles.fgaTitle}>{label}</Text>
      <View style={styles.row}>
        <View style={{flex: 1}}><Input label="CO (ppm)" value={value.co} onChangeText={(next) => onChange({...value, co: next})} keyboardType="decimal-pad" /></View>
        <View style={{flex: 1}}><Input label="CO₂ (%)" value={value.co2} onChangeText={(next) => onChange({...value, co2: next})} keyboardType="decimal-pad" /></View>
        <View style={{flex: 1}}><Input label="Ratio" value={value.ratio} onChangeText={(next) => onChange({...value, ratio: next})} keyboardType="decimal-pad" /></View>
      </View>
    </View>
  );
}
      ...
      <KeyboardAvoidingView style={{flex: 1}} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={{paddingTop: insets.top + 8, paddingHorizontal: 20, paddingBottom: TAB_BAR_HEIGHT + 110}} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <FormSection title="Appliance Details" subtitle="Installation certificate for one appliance.">
            ...
            <View style={styles.row}><View style={{flex: 1}}><Input label="Make" value={form.make} onChangeText={(value) => updateField('make', value)} /></View><View style={{flex: 1}}><Input label="Model" value={form.model} onChangeText={(value) => updateField('model', value)} /></View></View>
          ...
          <FormSection title="Readings & Checks">
            ...
            <FgaGroup label="FGA low fire" value={form.fgaLow} onChange={(value) => updateField('fgaLow', value)} />
            <FgaGroup label="FGA high fire" value={form.fgaHigh} onChange={(value) => updateField('fgaHigh', value)} />
