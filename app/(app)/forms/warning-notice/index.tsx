...
export default function WarningNoticeIndex() {
  ...
  useEffect(() => {
    const loadSeed = async () => {
      try {
        const editRaw = await AsyncStorage.getItem(WARNING_NOTICE_EDIT_SEED_KEY);
        if (editRaw) {
          ...
          await AsyncStorage.removeItem(WARNING_NOTICE_EDIT_SEED_KEY);
          Alert.alert('Editing Warning Notice', 'Previous details have been loaded. Make your changes and save from Review.');
          return;
        }

        const duplicateRaw = await AsyncStorage.getItem(WARNING_NOTICE_DUPLICATE_SEED_KEY);
        ...
        await AsyncStorage.removeItem(WARNING_NOTICE_DUPLICATE_SEED_KEY);
        Alert.alert('Warning Notice Duplicated', 'Customer, property and appliance details have been prefilled. Review them before saving.');
      } catch {
        await AsyncStorage.removeItem(WARNING_NOTICE_DUPLICATE_SEED_KEY);
        await AsyncStorage.removeItem(WARNING_NOTICE_EDIT_SEED_KEY);
      }
  ...
  const handleNext = () => {
    ...
    router.push('/(app)/forms/warning-notice/hazard' as any);
  };
  ...
  return (
    <CustomerPropertyDetailsStep
      ...
      subtitle="Unsafe gas appliance warning notice"
      stepLabels={['Details', 'Hazard', 'Review']}
      nextButtonLabel="Next: Hazard Details"
